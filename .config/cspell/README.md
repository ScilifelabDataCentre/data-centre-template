...
3. _Optional (Recommended):_ Remove words from the `project-specific-words.txt` and `forbidden-words.txt` files. You can also start from scratch completely by removing the example entires (everything under the `# cspell:disable` line but above the `# cspell:enable` line) in those files.
